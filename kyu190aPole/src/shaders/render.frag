...
vec3 c1 = vec3(0.4, -0.2, 0.);
vec3 c2 = vec3(-0.3, .5, 0.);
vec3 c3 = vec3(-1, -1, 1);
vec3 c4 = vec3(-1, 1, 1);
// posX, posY, normalX, normalY
vec4 p1 = vec4(-1.4, 0, 1, 0);//vec4(-.4, 0, 1, 0);
vec4 p2 = vec4(0, -2., 0, 1);//vec4(0, -.3, 0, 1);
vec4 p3 = vec4((vec2(5., 5.)), normalize(-vec2(1, 1)));////vec4((vec2(.25, .25)), normalize(-vec2(1, 1)));
const int ITERATIONS = 100;
bool IIS(vec2 pos, out vec3 col){
    bool fund = true;
    float invCount = 0.;
    for(int i = 0 ; i < ITERATIONS ; i++){
      vec2 texPos = -vec2(.6, 0.35);
      vec2 size = u_textureResolution / 1600.;
      ...
      float dHalfPlane;
      pos -= p1.xy;
      dHalfPlane = dot(pos, p1.zw);
      invCount += dHalfPlane < 0. ? 1. : 0.;
      fund = dHalfPlane < 0.  ? false : fund;
      pos -= 2.0 * min(0., dHalfPlane) * p1.zw;
      pos += p1.xy;

      pos -= p2.xy;
      dHalfPlane = dot(pos, p2.zw);
      invCount += dHalfPlane < 0. ? 1. : 0.;
      fund = dHalfPlane < 0.  ? false : fund;
      pos -= 2.0 * min(0., dHalfPlane) * p2.zw;
      pos += p2.xy;

      pos -= p3.xy;
      dHalfPlane = dot(pos, p3.zw);
      invCount += dHalfPlane < 0. ? 1. : 0.;
      fund = dHalfPlane < 0.  ? false : fund;
      pos -= 2.0 * min(0., dHalfPlane) * p3.zw;
      pos += p3.xy;
...
void main() {
  ...
  float numSamples = 10.;
  float zoom = 0.;
  vec2 translate = vec2(-.2, 0);

  float t = mod(u_time, 22.);
  float start = 3.;
  // zoom -= 5.;
  ...
  p1.x += mix(0., 1., circEasingIn(scene(t, start, 1.)));
  p2.y += mix(0., 1.7, (scene(t, start + 1.5, 1.5)));
  p3.xy -= mix(0., 4.75, (scene(t, start + 3.5, 2.)));
  zoom -= mix(0., 4., (scene(t, start + 5.5, 1.)));
  float theta = mix(0., 2. * PI, scene(t, start + 6.5, 4.));
  ...
  p3.xy = p3.xy * m;
  p3.zw = p3.zw * m;
  c2.z += mix(0., .4, circEasingIn(scene(t, start+ 10.5, 1.)));
  c1.z += mix(0., .4, circEasingIn(scene(t, start+ 11.5, 1.)));

  zoom += mix(0., 4., (scene(t, start + 12.5, 1.)));
  float theta2 = mix(0., 2. * PI, scene(t, start + 12.5, 4.));
  mat2 m2 = mat2(cos(theta2), -sin(theta2),
  ...
  p3.xy = p3.xy * m2;
  p3.zw = p3.zw * m2;
  c2.z -= mix(0., .4, circEasingIn(scene(t, start+ 16.5, 1.)));
  c1.z -= mix(0., .4, circEasingIn(scene(t, start+ 16.5, 1.)));

  p1.x -= mix(0., 1., circEasingIn(scene(t, start + 17.5, 1.)));
  p2.y -= mix(0., 1.7, (scene(t, start + 17.5, 1.5)));
  p3.xy += mix(0., 4.75, (scene(t, start + 17.5, 2.)));
